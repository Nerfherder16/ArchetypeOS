...
export type Job = {
  ...
  project_id: string | null;
  repository_id: string | null;
  // AOS-NODE-EXECUTION-001: routing decision fields, surfaced for the Control Tower
  // audit. All carry default values in the backend so they always appear in the
  // response.
...
export type Connector = {
  ...
  enabled: boolean;
  configured: boolean;
  // AOS-CONNECTOR-RUNTIME-001: decomposed status — credential present vs actually
  // reachable (a non-empty default URL may be configured but the service may be
  // down). Surfaces the two failure modes independently for the UI.
