...
**Phase 2 landed (AOS-DISTILL-002) — code-aware distillation.** Per operator feedback that "reading the readme doesn't really nail down its usefulness … the nuance lays elsewhere like the codebase, how each component works, what it was built for," `distill_repository` now reads the actual **source** (bounded, meaningful) in addition to the README. `select_source_files(repository, *, dna, cap_files=10, cap_bytes=40_000)` picks entry-point files + the largest **source-classified** files (Phase-B `LANGUAGE_CLASS`) + the primary manifest, read via `safe_repo_path`, capped and tolerant (an unreadable/binary/absent repo yields `[]`, never raises). Two code-derived layers mirror the RFC-0005 deterministic/real-provider split: (1) `summarize_sources` — a **pure, stdlib** (`ast` + regex) structural summary emitting a per-file-cited `## Components (from source)` section (hermetic, CI-tested); (2) `reason_over_source(files, provider)` — a **reasoned narrative** (`## How it works / Built for`, citing files) produced **only** for a real, LES-021-isolated `claude_code` provider and parsed with the council's tolerant `_loads_tolerant`; the `deterministic` provider fabricates nothing (empty narrative), so CI never calls a live model. No new DB table/migration and no frontend — the existing `KnowledgePage`/vault-markdown read path carries the new sections. The live narrative is Orchestrator-live-validated (not CI-reproducible). Still deferred (see Non-goals): the Knowledge Transfer Engine, embeddings/semantic index, and web tools.
