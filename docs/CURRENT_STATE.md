...
- **AOS-DISTILL-001 (PR open) — RFC-0008 MVP: repository content extraction.** `distill_repository` reads a repo's README (deterministic, LLM-free) → provenance-tagged `wiki/repositories/<slug>.md` + re-syncable `KnowledgePage` (`page_type="repository"`) + `DNA.purpose`; a council selector surfaces it; `POST /repositories/{id}/distill`. Verified on the real free-llm clone (→ named catalog page, idempotent, sync re-derives). This is the **pipe + deterministic README floor**. **Operator feedback: README alone under-captures usefulness (the nuance lives in the codebase — how components work, what it was built for) → code-aware distillation (RFC-0008 Phase 2: read bounded source with the now-isolated `claude_code` provider, provenance-tagged) is the explicit next package, building on this MVP.** Prior: AOS-LLM-ISOLATION-001 merged (PR #60, LES-021 closed — provider now context-isolated, the enabler for Phase 2's code reading).
