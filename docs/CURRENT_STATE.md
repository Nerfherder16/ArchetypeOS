...
- **RFC-0008 Phase 2 — code-aware distillation (starting).** AOS-DISTILL-001 merged (PR #61): the distillation pipe + deterministic README floor is live (`distill_repository` → `wiki/repositories/<slug>.md` + re-syncable `KnowledgePage` + `DNA.purpose` + council selector; verified on the real free-llm clone). **Operator's key request drives the active build:** README alone under-captures usefulness — the nuance lives in the codebase (how components work, what it was built for). **Phase 2 reads bounded, meaningful source** (entry points + core modules identified via the Phase-B architecture graph) **with the now-isolated `claude_code` provider (LES-021)** to reason about component behavior + purpose, **provenance-tagged to files**, appended as a code-derived section on the repository page; the deterministic README path stays as the hermetic fallback.
