...
- Status: closed — reworded body cleared the BLOCK; recorded here per the "lesson for every Guardian BLOCK" rule. A possible hardening follow-up (NOT done here): make `check_override_lesson_citation` match an actual override claim (e.g. `PR_GUARDIAN_OVERRIDE_<KEY>` on its own line or with a rationale) rather than a bare substring, and/or accept the `LES-L\d+` id form. Track separately if pursued.
