...
AOS-EMBED-002 (RFC-0010 Part 2, real embedder) PR being opened. Prior merged: #71 (laptop union merge driver, `88aff19`), #70 (AOS-EMBED-001, `6833440`), #69/#67/#66/#65/#64/#63; #68 (laptop AOS-20 doc-staleness). Branch **rebased onto `origin/main` `88aff19`** (the laptop's #68/#71 integrated; union driver auto-merged the shared coordination logs). **RFC-0009 embeddings: Part 1 merged; Part 2 (real embedder) reconsidered before building → fastembed (ONNX), NOT sentence-transformers/torch (same MiniLM/384-dim, ~50 MB vs GBs; enables a real embedder CI job). Orchestrator-verified incl. live embedder-quality validation (semantic surfaces paraphrase matches the lexical floor scores at zero); no fastembed/onnxruntime/torch on `import aos_core.embeddings`; api 196 (+3 skips) / worker 7. Next: CI green (incl. the new "Embedder tests" + "Vector store" jobs) → Manual Merge Gate → operator merge; then the embedding tier is complete and the Control Tower "Reuse" view is the natural next increment. Tandem: laptop session merged AOS-20; shared board = Plane AOS. LES watermark: next lesson is LES-027 (laptop added 024/025/026).**
