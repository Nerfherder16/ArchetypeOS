...
- **AOS-DISTILL-002 (PR open) — RFC-0008 Phase 2: code-aware distillation.** The operator's key request delivered: distillation now reads the actual **source** (bounded: entry points + largest source files + manifest, capped, tolerant), not just the README. A deterministic `## Components (from source)` section (pure `ast`+regex, CI-tested) + an optional isolated-`claude_code` `## How it works / Built for` narrative (real provider only; deterministic fabricates nothing) — both provenance-tagged to files. **Live-validated on the real free-llm `src/data.py` → a grounded, code-cited narrative naming `MODEL_TO_NAME_MAPPING` (the mechanism the README omits), zero contamination (LES-021 holds).** No migration/frontend. Prior: AOS-DISTILL-001 (PR #61, the pipe + README floor); LES-021 (PR #60, the isolation that makes code reading safe); Phase B (PR #59, the architecture graph that guides source selection).
