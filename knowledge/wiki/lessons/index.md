...
| [[LES-027]] | 2026-07-06 | ci-failure | A malformed `ci.yml` fails the whole workflow with 0 jobs / 0 PR check-runs (invisible to ruff/compileall/compose-config); an unquoted single-line `run:` scalar containing `: ` (e.g. `print('ok: no torch')`) is parsed as a YAML mapping. Verify workflow YAML parses whenever a builder edits `.github/workflows/`; prefer `run: |` block scalars; treat "run=failure, 0 jobs" as a parse smell | AOS-EMBED-002 (PR #73) first CI run (run 28822124340) | closed | AOS-EMBED-002: line rewritten as `run: |` block scalar + de-coloned; `ci.yml` parses (8 jobs); "workflow YAML parses" added to the Orchestrator verification checklist |
