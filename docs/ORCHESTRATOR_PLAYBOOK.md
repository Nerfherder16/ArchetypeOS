...
## Design to the mature-state target (operator rule, 2026-07-06)

Standing directive from the operator ("we should always be looking at it from the mature state — why build things twice?"): **define the mature-state architecture of a subsystem first, then make every work package a strict *subset* of that target — a permanent layer that later work *extends*, never scaffolding you tear out.**

- The failure mode being avoided is **rework of *design*, not incremental delivery.** Shipping in small verified packages is correct; shipping a version whose *approach* gets replaced (not extended) is "building twice."
- **The test for every spec step:** *will this design be extended or torn out by the mature system?* If torn out, don't build it — or reduce it to the honest minimum that survives.
- **Before speccing a subsystem's packages, write its mature-state target** (the layers, and which tier owns which responsibility) so each `.archetype/work/<ID>.md` can name the subset it lands. Reject any step that isn't a subset of the target.
- **Canonical target pattern (already ratified — Council, distillation):** a **two-tier evidence pipeline** — a hermetic **deterministic floor** (CI-safe, offline, local-first, no API key) *plus* an **isolated-`claude_code` reasoned tier** (the primary quality source), with `validation_state` marking *derived* vs *reasoned*, over **deterministically extracted factual evidence** (languages/frameworks/services/edges) feeding both tiers. The floor stays **honest and conservative — never clever**: it strips noise and, when unsure, emits nothing (empty beats wrong); it does *not* try to rival the reasoned tier (that cleverness is the throwaway). Mature quality is the reasoned tier's job.
- Motivating arc: the distillation reality test (2026-07-06) → AOS-DISTILL-003 (floor + framework evidence) and AOS-DISTILL-004 (reasoned `DNA.purpose`) are two subsets of one target, sequenced back-to-back, not a patch-then-replace.
...
- **Partition shared sequential IDs by session, don't race them** (LES-L02): re-deriving from the live index is not enough — both sessions still read the same max and pick the same next `LES-NNN`, so whoever merges *second* gets an add/add collision (LES-027 on #74, LES-029 on #80). Fix: the **laptop session** allocates lesson IDs from the `LES-L##` band; the **cloud session** keeps the default `LES-NNN` sequence. Same principle as the union driver (LES-026) applied to a counter. Any new shared monotonic ID under concurrent authorship (work items, migrations) wants a per-session namespace or a single allocator.
