...
- Current sprint: Sprint 5 delivered (PRs #43–#48; AOS-18 Done). Intelligence Phase 1 (AOS-COUNCIL-001, PR #49, AOS-19). AOS-PORTFOLIO-001 merged (PR #53 — 5-repo reality test, AOS-21 Done). Intelligence arc: AOS-COUNCIL-PHASEA (PR #54), PHASEC (PR #55), PHASEC2A (PR #56), PHASEC2B (PR #57) — **Phase C COMPLETE end to end**. RFC-0008 (content extraction) proposed (PR #58). AOS-ARCH-SEMANTICS-001 merged (PR #59 — Phase B: compose/service architecture edges + source-classified language weighting; LES-013 closed, LES-014 compose half done). **The founding "feed a repo → extract what's useful → Obsidian for reuse → surface what's useful *for* a target" arc is now mature end-to-end.** Merged: AOS-LLM-ISOLATION-001 (PR #60, LES-021 provider isolation), AOS-DISTILL-001 (PR #61, RFC-0008 MVP README distillation), AOS-DISTILL-002 (PR #62, Phase 2 code-aware distillation), AOS-TRANSFER-001 (PR #63, RFC-0009 Knowledge Transfer Engine MVP). Then the **first end-to-end reality test** (2026-07-06) showed the loop connects but distillation evidence quality was the bottleneck, driving the operator "design to the mature-state target" rule (`ORCHESTRATOR_PLAYBOOK.md`) and three layered packages: AOS-DISTILL-003 (PR #64 — deterministic floor + framework evidence), AOS-DISTILL-004 (PR #65 — reasoned `DNA.purpose`, live-validated), AOS-TRANSFER-002 (PR #66 — transfer need-coverage calibration). The full reality test now confirms the loop returns the correct repo #1 on every query with meaningful confidence. RFC-0009 embeddings underway: **RFC-0010 merged (#69); AOS-EMBED-001 (Part 1 — vector-store + retrieval infra, pgvector, NO torch) merged (#70, `6833440`); AOS-EMBED-002 (Part 2 — real embedder) in review** — reconsidered before building to **fastembed (ONNX)** not sentence-transformers/torch (same MiniLM/384-dim, ~50 MB vs GBs), live-validated: semantic search surfaces paraphrase matches the lexical floor scores at zero (claude-agent-sdk on "build AI agents that call tools": lexical 0.000 → semantic 0.533). Plane AOS-25. The Control Tower "Reuse" view has now shipped (AOS-UI-001, PR #76). **Tandem laptop session shipped AOS-20 (doc-staleness, PR #68) + a union merge driver (PR #71, LES-024/025/026) into main; my branch rebased onto `88aff19` cleanly.** **Now merged: AOS-COUNCIL-002 (PR #74 — Agent Council dashboard, Control Tower read surface); AOS-SCAN-PRECISION-001 (PR #75 — scanner precision: JVM/.NET/rust manifest breadth + secret-signal fixture awareness, closes LES-016/LES-017); AOS-UI-001 (PR #76 — the Control Tower design system + live Reuse view: a scoped `.aos-*` ops-deck layer (blue+red tokens, self-hosted Bebas Neue, HUD/neumorphic/signal-meter primitives, both themes, inert against the existing page) + `ReuseView` wired to the live `POST /projects/{id}/transfer` endpoint — ranked evidence-backed reuse cards with a signal-strength confidence meter). The WebGL radar instrument shipped — #79 AOS-UI-002 (WebGL radar for the Reuse view, react-three-fiber); the full rail-shell migration (AOS-UI-003) remains the deferred next step (a strict superset).** **Now in progress: AOS-OPS-001 (CI-green signal) — a `ci-green` fan-in job that comments on all-green so a watching session is event-woken instead of polling.** Orchestration Opus 4.8.
...
- PR #74: Agent Council dashboard (AOS-COUNCIL-002) — a project-scoped "Agent Council" Control Tower section surfacing the full council reasoning the API already returned but the UI discarded (verdict badge with a distinct abstention render, confidence, question, provider; expand → Final Judge panel + per-agent cards). Frontend-only; new `apps/web/e2e/council-dashboard.spec.ts`. First frontend package on the union-merge-protected coordination logs (LES-026, LES-028).
- PR #75: Scanner precision (AOS-SCAN-PRECISION-001) — manifest/ecosystem breadth (JVM `pom.xml`/`build.gradle`/`build.gradle.kts` → `jvm`, .NET suffix `.csproj`/`.sln` → `dotnet`, both added to `ECOSYSTEM_KINDS`; rust `Cargo.toml` already covered) + test-fixture-aware `SECRET_LIKE_FILENAME` (downgraded to `info`, out of `risk_flags`/DNA, under `testdata`/`tests`/`fixtures`/…). Closes **LES-016** + **LES-017**; scanner-only. The guardian's separate overridable secret-block is intentionally unchanged (never-weaken).
- PR #76: Control Tower design system + live Reuse view (AOS-UI-001) — a new scoped `.aos-*` design-system layer (`apps/web/src/design/tokens.css`: blue+red tokens, self-hosted Bebas Neue, angular HUD frame / neumorphic chips / signal-meter primitives, both themes, scoped under `.aos-surface` so it stays inert against the existing page) + `apps/web/src/features/reuse/ReuseView.tsx` wired to the live `POST /projects/{id}/transfer` endpoint (RFC-0009 Knowledge Transfer Engine) — ranked evidence-backed reuse cards with a signal-strength confidence meter, matched-term chips, expandable reason/evidence/required-changes/risks, provenance; `api.ts` gains `TransferRecommendation` + `fetchReuseRecommendations`; new real-API `apps/web/e2e/reuse.spec.ts`. Guardian PASS, all 8 CI checks green, Orchestrator-verified. The WebGL radar instrument (AOS-UI-002) + the full rail-shell migration (AOS-UI-003) are the deferred next UI increments (each a strict superset).
- PR #79: WebGL radar instrument for the Reuse view (AOS-UI-002) — a react-three-fiber radar on the `.aos-*` design system that plots the live reuse candidates. New `apps/web/src/features/reuse/radarLayout.ts` (pure deterministic candidate→polar mapping, distance = `1 − confidence`) + `Radar.tsx` (r3f `<Canvas>` — rings, rotating sweep, pulsing core, tier-colored blips: cyan = has matched_terms, periwinkle = lexical-lean; active blip enlarged, hovered pulses; WebGL capability probe + error boundary → static placeholder fallback; prefers-reduced-motion freezes). `ReuseView.tsx` lifted card-expand state up so the radar drives it: click a blip → expand + scroll its card; hover a card → highlight its blip. Deps `three@0.171.0` / `@react-three/fiber@9.6.1` / `@types/three`; new e2e `radarLayout.spec.ts` + `reuse-radar.spec.ts`. Guardian PASS_WITH_WARNINGS (acknowledged `scanner-new-ecosystem` — the three/r3f dep add, still Node ecosystem); all 8 CI checks green; Orchestrator-verified (build clean, 15/15 web e2e, real-app screenshot). Reuse view only; generic reusable `<Radar>` + rail-shell migration remain AOS-UI-003.
- PR #81: CI-green signal (AOS-OPS-001) — a `ci-green` fan-in job (`.github/workflows/ci.yml`) that runs only when all PR jobs succeed and posts a `✅ CI green @ <sha>` comment, so a watching session is woken by the comment (webhooks deliver comments but not CI-success) — replacing timed polling; self-validated on its own PR. Carried the AOS-UI-002 reconciliation + two flake fixes (LES-030 decision-row `data-testid`; LES-031 finalize PR body before the triggering push). **AOS-UI-003 shipped.**
- PR #82: Control Tower rail shell + state-based view routing (AOS-UI-003) — the full-page adoption of the `.aos-*` ops-deck: the 11 stacked sections become 8 routed views behind a left rail + topbar + workspace (`activeView`, no router dep); the active-project selector moves to the rail foot; `apps/web/src/shell/Shell.tsx`; `main.tsx` restructured (state/handlers unchanged); `.aos-legacy` quarantines un-restyled sections legibly; all e2e navigate via `nav-<id>`. **Per-view restyle arc: AOS-UI-004 (#84 — Overview + Repositories) + AOS-UI-005 (#87 — Architecture/Knowledge/Digest/Scheduling) merged; AOS-UI-006 (Council & Decisions) in review COMPLETES the arc — the whole Control Tower is now on the native `.aos-*` ops-deck (badges → pills; only the shared select-project notice stays legacy). Next candidate track: landing page + TTS/STT voice agent.**
