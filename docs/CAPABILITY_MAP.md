...
- Framework detection from manifest bodies (AOS-DISTILL-003: a deliberate, bounded extension of the "reads no bodies except compose" rule — the scanner now reads the BODIES of detected `package.json` (deps/devDeps), `requirements.txt`, `pyproject.toml` (PEP 621 + poetry dependencies), and `go.mod` (module + requires), byte-capped and tolerant (try/except → skip), and maps a small CURATED table of well-known frameworks — fastapi/flask/django/starlette/pydantic/pydantic-ai/langchain, express/react/next/vue/angular/svelte/nest, gin/echo/fiber/…, and a few more — to normalized names in a new sorted, deduped `frameworks: list[str]` scan key; conservative (only high-confidence curated matches; unknown deps ignored). `run_scan` stamps `RepositoryDNA.frameworks`, feeding the Transfer Engine's technology-match boost. Partially addresses LES-016 (manifest-body deps for python/node/go frameworks); LES-016 stays open for the dotnet/jvm/cargo ecosystem breadth this does not cover)
...
- Knowledge Transfer Engine (RFC-0009 MVP **shipped** (AOS-TRANSFER-001): `recommend_reuse(db, *, need, exclude_project_id=None, limit=5)` retrieves the portfolio's distilled repositories relevant to a free-text **target need** and returns ranked, provenance-tagged reuse recommendations. Deterministic lexical relevance (hermetic, no model/network): tokenizes the need + each candidate's `KnowledgePage.title` + `RepositoryDNA.purpose`, scores by normalized token overlap (Jaccard) with a technology-match boost over the DNA's `language_mix`/`package_managers`/`frameworks`; drops zero-score matches and the target project's own repos; each recommendation carries source repository / reusable asset / matched-term reason / evidence (the source distillation's `vault_path` + repo id) / heuristic required-changes + risks / confidence = the relevance score. Advisory + compute-and-return (no new table/migration; a human promotes a chosen one into the `Recommendation`/`Decision` loop). Exposed as `POST /projects/{project_id}/transfer`. Embeddings/semantic relevance + a provider-reasoned adaptation plan are deferred behind the `score_relevance` seam.)
