...
- Goal: the first portfolio reality test — run the pipeline on **two** repos the system did not write (`pydantic/pydantic-ai` + `anthropics/claude-agent-sdk-python`, operator-chosen) + a repeatable repo-acquisition capability (the missing clone step, dogfooded on the SDK's real network clone). Deliverables: `clone_repo` (`aos_core/services/onboarding.py`) + `scripts/onboard_repo.sh` + tests; captured scans (`.archetype/portfolio/{pydantic-ai,claude-agent-sdk-python}/scan.json`) + evaluation (`docs/PORTFOLIO_PYDANTIC_AI.md`); **honest lessons LES-013 (file-count language mix misreads a library — repo-dependent: pydantic-ai 28% Python vs the lean SDK's correct 77%) + LES-014 (architecture edges tree-only, confirmed on both — Fable-flagged follow-up)**, both open. Does NOT fix the two gaps (scoped follow-ups per the Plane issue). LES-015 (self-caught e2e count-race) recorded closed.
