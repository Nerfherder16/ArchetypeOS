...
- Status: closed — the **fragment split** (the stronger remedy this lesson proposed) is implemented in AOS-DOCS-CAPMAP-SPLIT: `docs/CAPABILITY_MAP.md` is now a thin index and each layer lives in its own `docs/capability-map/layer-NN.md`, so concurrent branches recording capabilities in *different* layers touch *different* files and no longer collide. `tools/pr_guardian.py` `check_capability_map` now accepts a touch to the root index OR any layer file (and treats the layer files as the map, not unmapped new docs). This also removes the per-read token cost of the 570-line monolith. Residual same-line edits within one layer (renames/re-orders) remain single-writer by reconciliation-owner discipline (the LES-026 caveat); a `merge=union` mark on the individual layer files is an optional further softener if append-order conflicts ever recur.
