...
- Refinement (slice 2, project-context extraction): the local e2e env-flake set is **non-deterministic run to run**, not a fixed list. A clean-main baseline showed 8 failures; the post-change run showed 4 (a strict subset), because worker/scheduler/data-seed timing (`serve-api.sh` stub has no worker) makes `decisions`/`digest`/`nodes-view`/`reuse` flap while `council-dashboard`/`decision-loop`x2/`scheduling` fail every time. So do NOT assert "same N failures" — instead confirm (a) every post-change failure is within the known worker/scheduler/data-seed family, and (b) every spec that would actually catch your change's regression (for a shell/routing refactor: `repositories`, `reuse`, `shell-modes`, `hash-routing`, `control-tower`, `architecture-studio`, `nodes-view`) passed. Subset-of-baseline + all-relevant-specs-green is the real green, not an equal failure count.
