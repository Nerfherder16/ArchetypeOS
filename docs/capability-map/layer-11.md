...
- hardened authority envelope (AOS-AUTHORITY-HARDEN-001: the envelope is now server-classified, one-use, bound, and resumable. `action_class` is derived SERVER-SIDE from the `job_requirements` registry (never the client — a write/egress handler can't be submitted as `read_only`); an unknown `job_type` is rejected before persistence (422). `consume_action` is an ATOMIC one-use compare-and-swap (`authorized`→`executed`) that stamps the consuming `job_id` — concurrent consumers yield exactly one winner (proven on PostgreSQL), and enqueue consumes the envelope in the job-creation transaction. The `ActionRequest` is bound to `repository_id`/`payload_digest` (`matches()` — an approval for one repo/payload can't authorize another) + carries an `expires_at`. Distillation derives its egress sensitivity from the REPOSITORY's persisted `sensitivity` policy (a private repo requires approval, not hardcoded public) and supports approve-and-resume: a blocked call returns the pending id, the operator approves, a retry passes `action_request_id` (no fresh pending envelope per try), and the envelope is consumed only AFTER the action succeeds — never on failure)
- web dashboard
- GPU node
- WSL node
