...
- External repository evaluation register — "keep pile" (AOS-59..68; evaluated so far: claude-obsidian, claude-video, T3MP3ST, notebooklm-py, memtrace-public, obsidian-skills. `knowledge/wiki/repositories/index.md` + one page per externally-evaluated repo records verdict, borrow candidates, and links to the full teardown in `docs/repo-research/<repo>.md` and the Plane "External Repo Evaluation & Adoption Pipeline" tracking item. Distinct from the Distillation Engine — that extracts content from *onboarded* repos, this records *evaluation judgment* on candidate repos — but they share the `wiki/repositories/` directory, so `sync_knowledge` derives register entries as `page_type="repository"` too. Verdict vocabulary: adopt / partial-borrow / reject / monitor.)
